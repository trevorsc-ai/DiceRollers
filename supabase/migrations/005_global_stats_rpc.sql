-- ============================================================
-- Global stats RPC — bypasses RLS to aggregate all rolls
-- ============================================================
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'top_drinks', (
      ...
      FROM (
        SELECT drink_name, SUM(cnt)::int AS count
        FROM (
          SELECT red_drink_name AS drink_name, COUNT(*)::int AS cnt FROM rolls GROUP BY red_drink_name
          UNION ALL
          SELECT white_drink_name AS drink_name, COUNT(*)::int AS cnt FROM rolls GROUP BY white_drink_name
        ) sub
        GROUP BY drink_name
        ORDER BY count DESC
        LIMIT 8
