-- ============================================================
-- Sort achievement emojis by rarity (fewest unlocks = rarest = first)
-- ============================================================

-- Update get_global_stats() flair ordering
    ...
    'leaderboard', (
      ...
      FROM (
        SELECT
          ...
          COALESCE(
            (
              SELECT jsonb_agg(a.emoji ORDER BY rarity.unlock_count ASC, a.sort_order ASC)
              ...
              WHERE ua.user_id = p.id AND ua.completed_at IS NOT NULL
            ),
            '[]'::jsonb
          ) AS flair
        FROM rolls r
...
-- ============================================================
-- Update get_user_profile_stats() achievement ordering
-- ============================================================
...
BEGIN
  ...
  SELECT json_agg(
    ...
    ORDER BY rarity.unlock_count ASC, a.sort_order ASC
  )
  INTO v_achievements
  FROM user_achievements ua
...
-- ============================================================
-- New RPC: get_achievement_rarity()
-- Returns unlock counts per achievement for the achievements page
-- ============================================================
...
AS $$
  SELECT a.id, COUNT(ua.id), (SELECT COUNT(*) FROM profiles)
  FROM achievements a
  LEFT JOIN user_achievements ua ON ua.achievement_id = a.id AND ua.completed_at IS NOT NULL
  GROUP BY a.id;
$$;
