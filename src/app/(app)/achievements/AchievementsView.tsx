...
import {
  achievementsPageQueryKey,
  fetchAchievementsPage,
  type AchievementWithProgress,
  type PunchCardInstance,
...
export default function AchievementsView({ userId }: { userId: string }) {
  ...
  // Clear the "new_achievements" hint so the modal doesn't reappear on revisit.
  useEffect(() => {
    try {
  ...
  const { data, isLoading } = useQuery({
    queryKey: achievementsPageQueryKey(userId),
    // Catalog + progress change rarely during a session — extend stale window.
    staleTime: 2 * 60_000,
    queryFn: () => fetchAchievementsPage(supabase, userId),
  });

  const achievements = data?.achievements ?? [];
  const rarityByAchievement = data?.rarityByAchievement ?? {};
                ...
                <div className="space-y-2">
                  {items.map((a) => (
                    <AchievementCard
                      ...
                      achievement={a}
                      earned={isEarned(a)}
                      rarityCount={rarityByAchievement[a.id]}
                    />
                  ))}
                </div>
              )}
